...
public class EmailController {
    ...
    @PostMapping
    public String sendEmail(@Valid EmailData email, BindingResult bindingResult, Model model){
        if(bindingResult.hasErrors()){
            model.addAttribute(email);
            return "/";
        }
        emailService.sendMessage(EMAIL_CONTACT, "Formularz kontaktowy", emailService.getMessageFromEmailClass(email, false));
